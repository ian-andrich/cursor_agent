...
class WeatherTool(BaseTool):
    def __init__(self):
        super().__init__(
            name="weather", description="Get current weather for a US city"
        )
        load_dotenv()
    ...
    def parameters(self) -> Dict[str, Dict[str, Any]]:
        return {
            "city": {
                "type": "string",
                "description": ("US city name (can include state e.g. 'Portland, OR')"),
                "required": True,
            }
        }
    ...
    async def execute(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient(
                verify=ssl_context, timeout=httpx.Timeout(10.0)
            ) as client:
                url = f"{self.base_url}/current.json"
                params = {"key": self.api_key, "q": location_query, "aqi": "no"}
                ...
                try:
                    ...
                    raise WeatherAPIError(f"Failed to connect to API: {str(e)}")
                except httpx.TimeoutException:
                    raise WeatherAPIError("Request timed out")
                except KeyError as e:
                    raise WeatherAPIError(f"Missing data in API response: {str(e)}")
